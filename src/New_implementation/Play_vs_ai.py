...
class ConnectFourTerminal:
    ...
    def ai_turn(self):
        ...
        neutral_state = env.change_perspective(self.state, self.current_player)
        action = self.opponent.search(neutral_state)
        self.state, self.result , self.done = self.env.step(action)
        ...
        while not turn_completed and self.running:
            ...
            if key in [curses.KEY_LEFT, ord('h')]:
                ...
                if self.env.is_valid_location(self.cursor_x):
                    self.state, self.result, self.done =self.env.step(self.cursor_x)
                    self.current_player = -self.current_player
                    self.turn += 1
