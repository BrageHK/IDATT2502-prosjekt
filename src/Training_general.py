...
class Trainer:
    def __init__(self, config):
        ...
        self.mcts = MCTS(env=config["env"], num_iterations=config["num_iterations"], NODE_TYPE=config["node_type"], model=self.model)
    ...
    def is_threshold(self):
        return self.config["node_type"] == NodeType.NODE_THRESHOLD or self.config["node_type"] == NodeType.NODE_THRESHOLD_LIGHTWEIGHT

    def save_data(self, iteration=None):
        if iteration is not None:
            subpath = lambda file: self.config["save_folder"] + "/" + self.config["node_type"].name + "/" + self.config["env"].__repr__() + "/" + file +"/" + file + f"_{iteration}.pt"
        else:
            subpath = lambda file: self.config["save_folder"] + "/" + self.config["node_type"].name + "/" + self.config["env"].__repr__() + "/" + file + "/" + file + ".pt"
    ...
    def load_data(self, iteration=None):
        ...
        if iteration is not None:
            path = lambda file: self.config["save_folder"] + "/" + self.config["node_type"].name + "/" + self.config["env"].__repr__() + "/" + file +"/" + file + f"_{iteration}.pt"
        else:
            path = lambda file: self.config["save_folder"] + "/" + self.config["node_type"].name + "/" + self.config["env"].__repr__() + "/" + file + "/" + file + ".pt"
        ...
        try:
            if self.is_threshold():
                ...
                self.policy_loss_history, self.value_loss_history = load_loss_history(path("loss_values"))
    ...
    def self_play(self):
        ...
        with mp.Pool(mp.cpu_count()) as pool:
            result_list = list(tqdm(pool.starmap(game_func, args_list), total=len(args_list)))
    ...
    def loss_alpha_zero(self, policy_logits, value_logits, policy_target, value_target): #
    ...
    def optimize(self, memory):
        ...
        for i in range(self.config["num_epochs"]):
            ...
            for batch_index in range(0, len(memory), self.config["batch_size"]):
                sample = memory[batch_index:min(len(memory) - 1, batch_index + self.config["batch_size"])]
                ...
                if not self.is_threshold():
                    loss = self.loss_alpha_zero(policy_output, value_output, policy_targets, value_targets.squeeze(-1))
                else:
    ...
    def create_folder(self, path):
        ...
        subpath = path + "/" + self.config["node_type"].name + "/" + self.config["env"].__repr__()
    ...
    def learn(self):
        ...
        iteration = self.config["starting_iteration"]+1 if self.config["starting_iteration"] is not None else 1
